...
def read_clean_file_name(folder: str = "put_your_excel_here"):
    ...
    Read exactly one .xlsx from `folder` and return:
      - df: pandas DataFrame read from sheet 'IBT FORMAT' with header=None
    ...
    Raises:
      ...
      - KeyError if the workbook doesn't contain a sheet named 'IBT FORMAT'
    """
    ...
    # Verify the required sheet exists before reading
    with pd.ExcelFile(file_path, engine="openpyxl") as xf:
        if SHEET_NAME not in xf.sheet_names:
            raise KeyError(
                f"Sheet '{SHEET_NAME}' not found in '{file_path.name}'. "
                f"Available sheets: {', '.join(xf.sheet_names)}"
            )
        df = pd.read_excel(xf, sheet_name=SHEET_NAME, header=None, engine="openpyxl")
